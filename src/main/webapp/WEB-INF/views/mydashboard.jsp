...
<body>
	...
	<spring:url value="/tclistdashboard/M?user=Y" var="mthListUrl" />
	<spring:url value="/tclistdashboard/W?user=Y" var="weekListUrl" />
	<spring:url value="/tclistdashboard/D?user=Y" var="dayListUrl" />
				...
				<div class="col-sm-6 col-md-4">
					<div class="thumbnail">
						...
						<div class="caption">
							...
							<h1>${todayCnt}</h1>
							<p>
								<a role="button" class="btn btn-primary" onclick="location.href='${dayListUrl}'" >Details</a>
							</p>
						</div>
					</div>
				</div>
			...
			<table class="table">
				...
				<c:forEach var="ex" items="${exList}" varStatus="status">
					<tr>
						...
						<td><a href="${executionDtlUrl}${ex.id}">${ex.runDateAsStr}</a></td>
						<td>${ex.runGroupId}</td>
						<td>${ex.successCnt}</td>
						<td>${ex.failureCnt}</td>
					</tr>
